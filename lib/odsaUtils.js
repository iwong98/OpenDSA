...
(function($) {
  ...
  $(document).ready(function() {
    ...
    //var speechText = "";
      // A function to speak the text written by umsg
    ...
    var soundSettings = function(){
      return function() {
        ...
        var $elem = $("<button class='soundOff'></button>");
        $elem.attr("voice", "false");
        // Control the button by setting the attributes
        var soundControl = function() {
          if($elem.attr("voice") === "true") {
            $elem.attr("voice", "false");
          }else{
            $elem.attr("voice", "true");
          }
          var synth = window.speechSynthesis;
        ...
        //Identifying Sound button container
        var closestContainer;
        // Sound button click event
        $elem.click(function() {
        //Setting current slide text
          closestContainer = $(this).closest(".jsavcontainer, #container").attr("id");
          if($(this).closest(".jsavcontainer").attr("id") === closestContainer) {
            //First SlideShow "inlineav" type
            $(this).closest(".jsavcontainer").trigger("jsav-message", [$(this).closest(".jsavcontainer").find(".jsavoutput").clone().find(".MathJax").remove().end().text(), "true"]);
          }else{
            //Second SlideShow "avembed" type
            $(this).closest("#container").find(".jsavcontainer").trigger("jsav-message", [$(this).closest("#container").find(".jsavoutput").clone().find(".MathJax").remove().end().text(), "true"]);
          }
        //Switching OFF other sound buttons in the other containers.
          $(".sound").not(this).each(function() {
            $(this).attr("voice", "false");
            $(this).attr("class", "soundOff");
          });
        //call narration
          soundControl.call();
          $(this).toggleClass("sound", $(this).attr("voice") === "true");
          $(this).toggleClass("soundOff", $(this).attr("voice") === "false");
        });
        return $elem;
      };
    };
    //Adding the sound button to selected JSAV containers
    $(".jsavsettings").filter(function() {
      return $(this).siblings(".jsavoutput").length !== 0 ||  $(this).closest("#container").find(".jsavcontrols").length !== 0;
    }).wrap("<span class='new'></span>");
    $(".new").append(soundSettings());
/////////////////////////////////////
    moduleName = settings.MODULE_NAME;
    ...
    $(window).load(function() {
      // handling "jsav-message event" to retrive the msg value
      var modifiedMsg;
      var closestContainer;
      $(".jsavcontainer").on("jsav-message", function(on, msg) {
        closestContainer = $(this).attr("id");
        setTimeout(function() {
        // Clear the message from any formatting tags
          modifiedMsg = msg.replace(/<[^>]*>/g, "");
          ...
          //speechText = msg;
          if ($("#" + closestContainer).find(".new").find("button").attr("voice") === "true") {
            textToSpeech(modifiedMsg);
          }
        }, 100);
      });
      $(".jsavcontainer").on("jsav-updatecounter", function() {
        if ($(this).find(".new").find("button").attr("voice") === "true") {
          var synth = window.speechSynthesis;
          var amISpeaking = synth.speaking;
          if(amISpeaking === true) {
            synth.cancel();
      ...
      $(".jsavbegin").click(function() {
        $(this).closest(".jsavcontainer").find("button").attr("voice", "false");
        $(this).closest(".jsavcontainer").find("button").attr("class", "soundOff");
      });
        // Stop with end
      $(".jsavend").click(function() {
        ...
        // Slides with "Run" button
      $("#run").click(function() {
          // handling "jsav-message event" toretrive the msg value
        $(".jsavcontainer").on("jsav-message", function(on, msg) {
          closestContainer = $(this).closest("#container").attr("id");
          // Clear the message from any formatting tags
          setTimeout(function() {
            modifiedMsg = msg.replace(/<[^>]*>/g, "");
            modifiedMsg = modifiedMsg.replace(/\$/g, "");
        ...
        $(".jsavcontainer").on("jsav-updatecounter", function() {
          if ($(this).closest("#container").find(".new").find("button").attr("voice") === "true") {
            var synth = window.speechSynthesis;
            var amISpeaking = synth.speaking;
            if(amISpeaking === true) {
              synth.cancel();
        ...
        $(".jsavbegin").click(function() {
          $(this).closest("#container").find(".new").find("button").attr("voice", "false");
          $(this).closest("#container").find(".new").find("button").attr("class", "soundOff");
        });
        $(".jsavend").click(function() {
          $(this).closest("#container").find(".new").find("button").attr("voice", "false");
          $(this).closest("#container").find(".new").find("button").attr("class", "soundOff");
        ...
        //Reset sound button with reset button
