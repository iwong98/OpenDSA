...
///////////////////////speech 
      var prevline="";
      var arr=Array.apply(null, Array(200)).map(Number.prototype.valueOf,-1);
      function textToSpeech() {
      synth = window.speechSynthesis;
      amISpeaking = synth.speaking;
      var u = new SpeechSynthesisUtterance();
    if(document.getElementById(SpeechId).getElementsByClassName("jsavoutput jsavline")[0].textContent !== prevline){
      u.text = document.getElementById(SpeechId).getElementsByClassName("jsavoutput jsavline")[0].textContent; 
      prevline = document.getElementById(SpeechId).getElementsByClassName("jsavoutput jsavline")[0].textContent; 
    }
    var lbl = document.getElementById(SpeechId).getElementsByClassName("jsavlabel");
    for(var i=0; i<lbl.length; i++){
          if(lbl[i].style.display === "block"){
              if(arr[i]!==i)
                {
                    u.text = u.text + "\n" + lbl[i].textContent;
                    arr[i]=i;            
    ...
    var soundSettings = function(){
      return function() {
        ...
        $elem.click(function() {
            prevline = "";
            arr=Array.apply(null, Array(200)).map(Number.prototype.valueOf,-1);
          if((SpeechId !== null)) {
            if(($(this).closest("div[class='ssAV jsavcontainer']").length!==0) && ($(this).closest("div[class='ssAV jsavcontainer']").attr("id") !== SpeechId)){
              $("#" + SpeechId).find("button").attr("voice", "false");
              $("#" + SpeechId).find("button").attr("class", "soundOff");
              synth.cancel();
            }
            if(($(this).closest("div[id='container']").find("div[class='avcontainer jsavcontainer']").length !== 0) && ($(this).closest("div[id='container']").find("div[class='avcontainer jsavcontainer']").attr("id") !== SpeechId)) {
              $("#" + SpeechId).closest("div[id='container']").find(".new").find("button").attr("voice", "false");
              $("#" + SpeechId).closest("div[id='container']").find(".new").find("button").attr("class", "soundOff");
              synth.cancel();
            }
          }
          if($(this).closest(".new").siblings(".jsavoutput").length !== 0) {
            SpeechId = $(this).closest("div[class='ssAV jsavcontainer']").attr("id");
          } else {
            if($(this).closest(".new").closest("#container").children(".jsavcontainer").length !== 0){
              SpeechId = $(this).closest("div[id='container']").find("div[class='avcontainer jsavcontainer']").attr("id");
            }
          }
          soundControl.call();
